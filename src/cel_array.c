...
CEL_API void free_cel_array (cel_array_t ** array, cel_release_callback_fn_t relfunc)
{
	if (array == NULL)
		return;
	if (*array != NULL)
...
CEL_API int cel_array_init (cel_array_t * array, int opacity)
{
	...
	if (opacity < 0)
		opacity = _cel_array_default_length;
 ...
 * @return  1 for success and 0 for failed
 */
CEL_API int cel_array_destroy (cel_array_t * array, cel_release_callback_fn_t relfunc)
{
	register int i;
...
CEL_API void cel_array_insert (cel_array_t * array, uint_t idx, void *value)
{
	register uint_t i;
	if (idx < array->size)
	{
 ...
 * @return  void *
 */
CEL_API void *cel_array_del_obj (cel_array_t * array, void *obj, cel_compare_fn_t cfunc)
{
 ...
 * @return  int
 */
CEL_API int cel_array_get_obj (cel_array_t * array, void *obj, cel_compare_fn_t cfunc)
{
	register uint_t i;
	for (i = 0; i < array->size; i++)
	{
 ...
 * @reutrn  void *
 */
CEL_API void *cel_array_set (cel_array_t * array, uint_t idx, void *value)
{
	void *old;
	if (idx >= array->size)
		return NULL;
...
CEL_API void *cel_array_set_obj (cel_array_t * array, void *obj, void *value, cel_compare_fn_t cfunc)
{
	int i = cel_array_get_obj (array, obj, cfunc);
	if (i == -1)
		return NULL;
...
{
	cel_intArray_t *ptr = (cel_intArray_t *) cel_malloc (sizeof (cel_intArray_t));
	if (ptr == NULL)
	{
...
CEL_API int cel_intArray_insert (cel_intArray_t * array, uint_t idx, int val)
{
	register uint_t i;
	if (idx >= array->size)
		return 0;
 ...
 */
CEL_API int cel_intArray_del_obj (cel_intArray_t * array, int val)
{
