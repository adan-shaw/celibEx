...
static int next_least3_repeat (char *inbuff, uint_t inlen, uint_t start, uint_t * len)
{
	...
	for (i = start; i < inlen;)
	{
		val = inbuff[i];
		p = 0;
		for (j = i; j < inlen && p < RLE_BUCKET_MAX_LENGTH && inbuff[j] == val; j++, p++) ;
...
CEL_API int cel_rle_encode_file (char *infile, char *outfile)
{
	...
	//Open the file.
	if ((infp = fopen (infile, "rb")) == NULL || (outfp = fopen (outfile, "wb")) == NULL)
	{
		return -1;
	}
	else
