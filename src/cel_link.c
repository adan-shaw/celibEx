...
static cel_link_node_t *create_link_node (void *, cel_link_node_t *, cel_link_node_t *);
static void insert_before (cel_link_node_t *, void *);
...
CEL_API cel_link_t *new_cel_link (void)
{
	...
	{
		cel_free (ptr);
		CEL_ALLOCATE_ERROR ("new_cel_link", sizeof (cel_link_node_t));
	}
 ...
 */
CEL_API int cel_link_init (cel_link_t * link)
{
	link->size = 0;
 ...
 * @param   _prev
 * @param   _next
 * @return  cel_link_node_t
 */
static cel_link_node_t *create_link_node (void *value, cel_link_node_t * _prev, cel_link_node_t * _next)
{
	cel_link_node_t *node = (cel_link_node_t *) cel_malloc (sizeof (cel_link_node_t));
	if (node == NULL)
		return NULL;
...
{
	cel_link_node_t *o = create_link_node (value, _old->_prev, _old);
	if (o != NULL)
	{
		o->_prev->_next = o;
		o->_next->_prev = o;
...
//static void cel_remove_node( cel_link_node_t node ) 
//{
//    node->_prev->_next = node->_next;
//    node->_next->_prev = node->_prev;
//}
 ...
 * @return  void *
 */
CEL_API void *cel_link_remove_obj (cel_link_t * link, void *value, cel_compare_fn_t comp)
{
	void *v;
	cel_link_node_t *o;

	for (o = link->head->_next; o != link->tail; o = o->_next)
	{
...
CEL_API void *cel_link_set_obj (cel_link_t * link, void *search, void *value, cel_compare_fn_t comp)
{
	void *v;
	cel_link_node_t *o;
