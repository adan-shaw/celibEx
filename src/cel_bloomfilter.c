...
{
	cel_bloomfilter_t *bloom = (cel_bloomfilter_t *) cel_malloc (sizeof (cel_bloomfilter_t));
	if (bloom == NULL)
	{
...
CEL_API int cel_bloomfilter_init (cel_bloomfilter_t * bloom, int length, int hfuncs)
{
	...
	if ((bloom->hfuncs = (cel_hash_fn_t *) cel_calloc (hfuncs, sizeof (cel_hash_fn_t))) == NULL)
	{
		cel_free (bloom->str);
		return 0;
	}
 ...
 * @return  int
 */
CEL_API int cel_bloomfilter_add_func (cel_bloomfilter_t * bloom, cel_hash_fn_t func)
{
	if (bloom == NULL)
		return 0;
